...
by as little more than an input to a problem. But the _people_ inside the cars
...
```{r}
#| label: fig-RoanokeTAZ
#| fig-cap: "Travel Analysis Zones in Central Roanoke."
...
Fill out the SE table from @tbl-setable with *just* this 
...
@fig-udot-classes shows roads in Provo and Orem classified by this
...
```{r}
#| label: fig-udot-classes
#| fig-cap: "UDOT functional classes."
...
*inputs* that can be used for developing travel models. Besides highway networks---which usually have to be supplied by the transportation agency---modelers
...
fill out a diary of their activities on an assigned day;
@fig-travel-diary shows an example of one activity from a survey diary.
...
@tbl-show-nhts-hh through @tbl-show-nhts-trips show
...
Note that that the households data in Table @tbl-show-nhts-hh contains a
...
The bottom three layers are shown in @fig-censusgeos. Each layer
...
a mean trip purpose, or at least one that makes sense.
